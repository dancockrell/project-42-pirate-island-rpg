...
| A | Ward Line | Place a ward between two adjacent bands; the first enemy crossing it takes damage and becomes Staggered. | **Implemented** |
...
- **A Ward Line** (`skill.ayla.ward_line`) — zero-selection normal action that places (or replaces) a single ward at Ayla's current band, emitting `BattleEvent::WardLinePlaced`. No hostile in this engine ever moves bands, so "the first enemy crossing it" is translated the same way Reach Counter translated "an enemy entering the Contested band": **the first hostile attack targeting a party member who shares the warded band**, checked inside `apply_damage` right alongside Reach Counter's structurally identical trigger. On trigger it emits `BattleEvent::WardLineTriggered`, deals `8 + Ayla's level` raw damage back to the attacker through Guard (a separate authored constant from Reach Counter's `10 + level`, since the two reactions have different eligibility costs), and applies a new `Staggered` status to the attacker via a new general-purpose `apply_status` helper — the engine's first genuinely *dynamic* status application; every other status before this was only ever set at actor construction or removed by a skill, never added mid-battle. The ward consumes itself on that first trigger and does not fire again until re-placed.
...
## Still blocked (2/7) — named, specific missing subsystems
...
- **SSS Override Tomb Rule** — same "once per expedition" scoping gap as Deny Activation. Unlike Ward Line, this isn't a same-battle translation problem: "until the party leaves the site" is expedition-level state, which is `ExpeditionState`'s domain (`godot-rust/src/expedition.rs`), not `Battle`'s. Implementing it means designing how an expedition-level rule override reaches into a `Battle` instance's resolution — a real architectural decision this document isn't making unilaterally.

## Why this isn't content yet

`tools/src/validate.mjs` requires every skill's `animation.eventBindings` to bind only to event kinds Rust's `Battle` actually emits, and requires `targetRule` to be one already registered with real Godot targeting behavior — and the character schema requires *exactly* seven working skill records to add a heroine at all, with no partial-roster path. The five implemented skills above could now honestly pass that bar; the two blocked ones still can't without the missing subsystems named above. Authoring all seven as content today, with the blocked two faked to fit the schema, would produce content that "validates" but is mechanically inert or misleading — the same failure mode the project's own placeholder-art discipline exists to prevent for art, applied to mechanics.

Once Deny Activation and Override Tomb Rule land in `battle.rs`/`expedition.rs`, `content/characters/ayla.json` and her seven `content/skills/ayla.*.json` records can be authored the same way Betty's were, with real animation beats bound to real events, and this document can be retired in favor of the code and `docs/ARCHITECTURE.md`.
