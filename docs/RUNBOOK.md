...
After validation, run `npm run build:content` from `tools/` or `node tools/src/build-content-bundle.mjs` from the repository root. Commit `game/generated/content_bundle.json` whenever its source records change. Rebuilding also writes one document per scenario pack to `game/generated/scenarios/<scenario-id>.json`; commit those with the pack that produced them. `ContentCatalog` loads only that bundle and returns defensive copies so callers cannot mutate the catalog's authoritative definitions.

Scenario packs live in `content/scenarios/<name>/scenario.json` and are described by `docs/SCENARIO_PACKS.md` and `content/schemas/scenario.json`. The validator resolves every path a manifest names, embeds nothing itself, and refuses a pack whose paths do not resolve, whose faction IDs are not declared in `content/factions/`, whose production rule names a `producerArchetypeId` absent from the pack's buildings document or an `outputDefinitionId` absent from its personas document, whose seed falls outside the navigation size or collides with another faction's seed, or whose reserved `resources`, `items`, `triggers` and `quests` arrays are not empty. Never hand-edit `game/generated/scenarios/`; edit the pack and rebuild.

Start a new pack with `npm run scaffold:scenario -- scenario.<name>` from `tools/` (or `node tools/src/scaffold-scenario.mjs scenario.<name>`). The skeleton references the main scenario's rule, buildings, personas and navigation documents by relative path, places two factions with their committed seeds and tuning, and validates as written. Add `--out <directory>` to scaffold outside `content/scenarios/`, then prove that pack with `node tools/src/validate.mjs --scenario-root <directory>`, which validates the repository plus every pack under the extra root.
