...
`Hold Position` (`skill.system.hold_position`) is the universal Guard verb: the one owner-neutral command every actor may spend. It takes no target, grants two Guard and ends the actor's turn. It is a real command, not scaffolding — A6 gave Captain Michael no Guard skill of his own precisely because this one already exists, and a test holds that he can spend it. What *is* provisional is the automatic prototype driver's habit of submitting it for a heroine whose authored kit is not yet executable; that stands in for her signature skills, never for Guard itself, and disappears the day her command set is implemented. The driver submits ordinary commands through `SimulationPort`; it never advances initiative or changes Guard in presentation code.
