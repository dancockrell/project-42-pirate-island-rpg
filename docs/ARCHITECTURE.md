...
### Executable production and dispatch foundation

`FactionWorld` in `godot-rust/src/world.rs` is the current executable owner of the first production-to-deployment path. `enqueue_production` validates the faction, operational producer, producer archetype, queue capacity, population capacity, and every resource cost before mutating state. It reserves all costs and population atomically. `advance_production_tick` advances operational queues and creates `ProducedActor` instances at their producer's node with immutable `ActorProductionProvenance`. A completed actor has no assignment until `dispatch_actor` evaluates legal candidates.

Dispatch uses integer score components and bounded integer wobble so replay does not depend on floating-point behavior. The highest total wins; equal totals use ascending stable action ID. The accepted assignment moves the actor to the selected network node, stores its action ID, and emits an event containing faction, assignment, target, and total score. Candidate generation, path legality, relationship-derived hatred, supply reach, weather, and terrain modifiers remain the next simulation layer; presentation is not permitted to fill those gaps.
