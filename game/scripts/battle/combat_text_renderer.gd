...
func render(event: Dictionary) -> Dictionary:
	...
	match kind:
		...
		"recovery_opening_created": return append(" [color=#d9a441]RECOVERY OPENING[/color] %s is exposed after %s. The next party hit gains %d raw damage." % [name_at(subjects, 0), skill_name(payload.get("source_skill_id", "enemy_action")), payload.get("bonus_raw_damage", 0)])
		"recovery_opening_consumed": return append(" %s punishes %s's recovery for %d bonus raw damage." % [name_at(subjects, 1), name_at(subjects, 0), payload.get("bonus_raw_damage", 0)])
		"recovery_opening_expired": return append(" %s regains its footing; the recovery opening closes." % name_at(subjects, 0))
