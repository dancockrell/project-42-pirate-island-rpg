...
func _init() -> void:
	...
	check(expedition.get_node("LocationPanel/LegalRouteList").get_child_count() == 1, "Black Beach must expose exactly one legal route")
	...
	check(expedition.get_node("LocationPanel/LegalRouteList").get_child_count() == 3, "River Landing must project its three legal native routes")
