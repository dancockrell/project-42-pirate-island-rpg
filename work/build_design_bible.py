...
add_p(doc,'A modern 3D party RPG, presented through a theatrical 2D interface, about dangerous exploration, an impossible tropical island, and the household that forms when people from incompatible histories choose to survive together.',after=20,align=WD_ALIGN_PARAGRAPH.CENTER,italic=True)
...
add_callout(doc,'Current design authority','docs/PIRATE_ISLAND_CONTINUATION_BRIEF.md (5 September 2026) is the current design authority. Where this bible and the brief disagree, the brief wins; registry entries this affects are marked SUPERSEDED in A.11. The brief keeps this bible\'s character-scale combat, tomb grammar, Midnight Return, household tone and asset discipline, and replaces its cast size, linear act structure, fixed duration and single-Champion threat with an autonomous six-faction simulation.')
...
('req.scope.cast.core','SCOPE','SUPERSEDED','The core cast is Captain Michael and four women (Pirate Island Continuation Brief, 5 Sep 2026). Betty and Ayla are established; the other two are open. The earlier six-name roster no longer applies.','test.scope.cast_manifest'),
('req.scope.campaign.duration','SCOPE','SUPERSEDED','The campaign is an organic path through an autonomous faction simulation with many victory, loss and mixed endings (Continuation Brief §15); there is no fixed duration target. Day 100 is an in-world escalation threshold, not a length.','test.telemetry.campaign_duration'),
...
('Decision','Presentation of adult intimacy.','Romancing your companions is a headline feature. The base game presents intimate scenes fade-to-black. An optional presentation-override pack, sold on adult storefronts, raises the same authored scenes to an explicit level; the pack changes what a scene shows, never what it does, and the simulation never reads presentation.'),
...
('Adult-content branching','Fixed participant combinations and pair histories can multiply scenes and assets.','Author intimacy around named milestones with fixed participant sets. One base presentation level (fade to black); the adult pack overrides scenes by ID rather than branching them.','Export a content matrix showing which scenes exist, their participants, and the exact milestone flags they change; list which scene IDs the pack overrides.')],[1.15,1.65,1.95,1.75])
...
('Campaign length','SUPERSEDED by the Continuation Brief: an organic path through an autonomous faction simulation, with no fixed duration target. Day 100 is an in-world escalation threshold, not a length.','External playtest still controls cuts. Optional completion may reach 38 hours.'),
('Campaign structure','SUPERSEDED: organic paths produced by the simulation, with many victory, loss and mixed endings.','Protected saves at strategic boundaries rather than at fixed chapter councils.'),
('Playable cast','SUPERSEDED: Captain Michael and four women. Betty and Ayla are established; the other two are open.','Five complete cards, battle actors, exploration portraits and relationship identities.'),
('Field party','Captain Michael and the four women: five heroes.','Five active cards.'),
...
add_callout(doc,'Protagonist','The protagonist is Captain Michael, a shipwreck survivor with history among Argentines and in Texas (Continuation Brief §5.1, §20). His name is fixed, not chosen at character creation. Restrained visual options and no fully voiced protagonist during the prototype still apply; the exact year, divergence and substance of his history remain open.')
...
add_p(doc,"The first campaign should be dense rather than an imaginary hundred-hour epic whose middle will never be produced. Its spine establishes Captain Michael's first strategic core, brings the four women into the household, opens the tomb network, exposes the Cthulhu faction's pattern of adaptation and forces a decision about the intelligence beneath the island. Because the island is an autonomous six-faction simulation, that spine is a set of pressures rather than a fixed act order: expeditions deepen factions, romances, companion pairs, routes, equipment and the ending reached. Each belongs to at least one enduring system, and the player is not required to exhaust them to reach an ending.")
...
add_h(doc,'7. The principal women',1)
add_callout(doc,'Cast superseded',"The Continuation Brief sets the cast at Captain Michael and four women. Betty and Ayla are established in the repository. The identities and origins of the other two are open (brief section 20). The four additional biographies below are retained as candidate material only; none of Vix, Grisha, Isabella or Nara is a confirmed member of the four, and no content record may be authored for them until the decision is recorded.")
heroines=[
