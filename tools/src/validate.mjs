...
for (const pack of scenarioPacks) {
  ...
  for (const field of ["resources", "triggers", "quests"]) {
  ...
  const catalogue = resolved.get("items");
  const catalogueIds = new Set();
  if (catalogue !== undefined) {
    if (typeof catalogue !== "object" || catalogue === null || Array.isArray(catalogue)) fail(file, `${name} items must be a keyed item catalogue document`);
    else {
