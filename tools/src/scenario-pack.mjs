...
export function declaredPaths(manifest) {
  ...
  for (const [index, path] of (Array.isArray(manifest.characters) ? manifest.characters : []).entries()) {
    declared.push({ field: `characters[${index}]`, path });
  }
  ...
  for (const [index, path] of (Array.isArray(manifest.leads) ? manifest.leads : []).entries()) {
    declared.push({ field: `leads[${index}]`, path });
  }
